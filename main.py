...
def path(last, visited):
    ...
    for dist in range(N):
        ...
        if visited & (1 << dist) == 0 and dists[last][dist] != 0:
            ...
            #         temp3 = temp3 & (temp3 - 1)
            #         count += 1
            #     print(last, dist, count, bin(visited), '  update :', temp2, '->', temp)
...
for i in range(N):
    for j in range(N):
        ...
        if distance - dists[current_place][j] == cache[j][visited_place + (1 << j)]:
            path_order.append(j)
            # print('path update', distance, current_place, j, cache[j][visited_place + (1 << j)])
            # print('path update', bin(visited_place), bin(visited_place + (1 << j)))
